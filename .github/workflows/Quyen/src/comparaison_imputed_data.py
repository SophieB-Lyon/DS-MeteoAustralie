...
def density_before_after_imputation(df_before: pd.DataFrame, df_after: pd.DataFrame,
                                    list_col: list,
                                    title: str = "Comparaison des densités"):
    fig, axes = plt.subplots(figsize=(20, 20), nrows=4, ncols=4)
    axes = axes.reshape(-1)
    sns.set(style="darkgrid")
    for i, col in enumerate(list_col):
        sns.kdeplot(df_before[col], fill=True, color='blue', ax=axes[i], label="Avant")
        sns.kdeplot(df_after[col], fill=True, color='green', ax=axes[i], label="Après")
