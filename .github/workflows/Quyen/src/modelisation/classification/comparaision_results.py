...
list_model = ["LinearRegression", "TreeDecision", "RandomForest", "XGBoost"]
...
for metric_name in list_metric_considered:
    ...
    for clf, label in zip([clf_lr, clf_tree, clf_rf, clf_xgboost], list_model):
        ...
        rocauc_rain = roc_auc_score(y_test_encoded, clf.predict_proba(X_test)[:, 1])
        rocauc_no_rain = roc_auc_score(y_test_encoded, clf.predict_proba(X_test)[:, 0])
